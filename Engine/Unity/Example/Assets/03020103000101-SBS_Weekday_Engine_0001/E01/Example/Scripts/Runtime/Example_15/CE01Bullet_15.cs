...
/** 총알 */
public class CE01Bullet_15 : CComponent
{
	#region 변수
	...
	public void OnTriggerEnter(Collider a_oCollider)
	{
		 ...
		 * 것이 불가능하다는 것을 알 수 있다.)
		 * 
		 * Unity 의 모든 게임 객체는 해당 객체가 속해 있는 레이어가 지정되어
		...
		{
			var oInteractable = a_oCollider.GetComponentInParent<CE01Interactable_15>();
			oInteractable?.OnHit();

			var oSceneManager = CSceneManager.GetSceneManager<CE01Example_15>(KDefine.G_N_SCENE_EXAMPLE_15);
			oSceneManager.ObjsPoolManager.DespawnObj<CE01Bullet_15>(this.gameObject, this.OnCompleteDespawnObj);
		}
	}
