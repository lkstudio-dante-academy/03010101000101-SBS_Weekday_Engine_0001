...
public class CComponent : MonoBehaviour {
	 ...
	 * 
	 * Awake 는 해당 컴포넌트가 사용 완료되는 즉시 호출 되는 반면 Start 메서드는 사용 완료 된 시점에 바로 호출
	 * 되는 것이 아니라 다음 프레임에 호출되는 차이점이 존재한다.
	 * 
	 * 따라서, 특정 객체를 생성과 동시에 초기화 시키는 작업을 수행하고 싶다면 Start 메서드 보다는 Awake 메서드를
	 * 사용하는 것을 추천한다.
	 */
	/** 초기화 */
