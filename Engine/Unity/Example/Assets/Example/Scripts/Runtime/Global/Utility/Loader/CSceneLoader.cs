...
public class CSceneLoader : CSingleton<CSceneLoader> {
	...
	public void LoadSceneAsync(string a_oName, 
		 ...
		 * 
		 * 유니티는 멀티 쓰레드 환경에 안전하지 않기 때문에 서브 쓰레드를 이용하면 비동기 처리를 추천하지 않는다. (즉, 서브 쓰레드에서
		 * 유니티 엔진과 관련 된 구문을 작성 할 경우 내부적으로 예외가 발생한다.)
		 * 
		 * 따라서, 유니티는 비동기 처리 구문을 작성 할 수 있게 코루틴 기능을 지원한다. (즉, 유니티에서 지원하는 비동기 처리 구문들은
