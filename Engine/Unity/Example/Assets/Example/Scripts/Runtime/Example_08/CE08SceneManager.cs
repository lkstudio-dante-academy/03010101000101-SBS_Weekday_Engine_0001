...
public class CE08SceneManager : CSceneManager {
	...
	public override void Update() {
		...
		if(Input.GetMouseButtonDown((int)EMouseBtn.LEFT)) {
			...
			if(Physics.Raycast(stRay, out RaycastHit stRaycastHit) && 
				...
				var oTarget = stRaycastHit.collider.GetComponent<CE08Target>();
	...
	/** 타겟을 캐치했을 경우 */
	private void OnCatchTarget(CE08Target a_oSender, int a_nScore) {
		var stPos = a_oSender.transform.position.ExToLocal(m_oScoreRoot);

		var oScoreText = CFactory.CreateCloneGameObj<CE08ScoreText>("ScoreText",
