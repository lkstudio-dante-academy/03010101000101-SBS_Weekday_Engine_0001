...
/*
 * 유니티 UI 시스템 종류
 * - ImGUI			<- 유니티 초기 버전부터 활용되던 UI 시스템
 * - Unity GUI		<- 현재 가장 많이 활용되는 UI 시스템
 * - UI Toolkit		<- 차세대 UI 시스템 (단, 현재 개발 진행 중)
 * 
 * ImGUI vs Unity GUI
 * - ImGUI 는 유니티 초기부터 현재 버전까지 활용되는 UI 시스템이기 때문에 해당 시스템을 활용하면
 * 모든 유니티 버전에서 구동되는 UI 를 제작하는 것이 가능하다.
 * 
 * 단, ImGUI 는 유니티 에디터 상에 눈으로 보고 배치하는 것이 불가능하며 100 % 스크립트만으로 처리
 * 되기 때문에 사용하기 굉장히 비효율적이고 까다롭다는 단점이 존재한다.
 * 
 * 따라서, 유니티는 Unity GUI 를 개발했으며 해당 시스템은 현재까지도 널리 활용되는 UI 시스템이다.
 * 
 * Unity GUI 는 UI 위젯들을 직접 눈으로 보고 배치 할 수 있기 때문에 ImGUI 에 비해서 UI 를 효율적으로
 * 제작하는 것이 가능하다.
 * 
 * 단, 유니티는 구동 시에 디스플레이 보여지는 UI 뿐만 아니라 유니티 에디터 자체의 UI 도 필요에 따라
