...
#if E23_ARRAY_LIST
/** Example 23 - 배열 리스트 */
public partial class CExample_23 : CSceneManager {
	#region 변수
	...
	protected override void UpdateState() {
		...
		for(int i = 0; i < nNumVals; ++i) {
			...
			if(i < m_oValList.Count) {
				...
				oTarget.GetComponent<CE23Target>().SetVal(nVal);

				// 변화가 발생한 값 일 경우
				if(i == m_nChangedIdx) {
					oTarget.GetComponent<CE23Target>().StartFadeAni(0.0f, 1.0f);
				}
			}
		}
	...
	private void TryRemoveVal(int a_nIdx) {
		...
		oTarget.GetComponent<CE23Target>().StartFadeAni(1.0f, 0.0f, (a_oSender) => {
