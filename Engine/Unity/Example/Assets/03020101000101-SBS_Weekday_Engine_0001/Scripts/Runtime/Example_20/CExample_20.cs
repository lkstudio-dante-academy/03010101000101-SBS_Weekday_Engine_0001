...
/*
 * Graphic Raycaster 란?
 * - 유니티가 제공하는 이벤트 시스템에 반응하는 객체를 탐색하는 역할을 수행하는
 * 컴포넌트를 의미한다. (즉, 해당 컴포넌트가 부착 된 캔버스 하위에 존재하는
 * UI 관련 컴포넌트는 유니티가 제공하는 각종 입력 이벤트에 반응하는 것이 가능하다는
 * 것을 알 수 있다.)
 * 
 * 또한, 유니티가 제공하는 각종 이벤트 인터페이스를 활용하면 이벤트 시스템에
 * 반응하는 커스텀한 스크립트를 제작하는 것이 가능하다. 
 * 
 * 단, Graphic Raycaster 는 UI 관련 컴포넌트에만 동작하기 때문에 
 * 스프라이트와 같은 메시 기반 컴포넌트가 유니티 이벤트 시스템에 반응하기 
 ...
 */
/** Example 20 */
public class CExample_20 : CSceneManager
{
	/** 상태 */
	private enum EState
	{
	...
	/*
	 * System.Serializable 속성은 구조체 또는 클래스를 유니티 인스펙터 상에
